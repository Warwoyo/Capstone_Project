...
<div class="max-w-xl mx-auto mt-20 p-8 bg-white rounded-lg shadow">
    ...
    {{-- ====================== FORM KHUSUS ADMIN & GURU ====================== --}}
    @php($isStaff = in_array(auth()->user()->role, ['admin', 'teacher']))
    ...
    @if ($isStaff)
        ...
        {{-- Flash sukses token --}}
        @if (session('success'))
            <p class="mb-4 text-green-600 text-center font-medium">
                {{ session('success') }}
            </p>
        @endif

        <form method="POST" action="{{ route('students.store') }}"
              class="space-y-4 max-w-lg mx-auto text-left">
            @csrf
            ...
            <button
                class="w-full sm:w-1/2 mx-auto block px-4 py-2 bg-sky-600
